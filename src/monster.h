...
public:
	...
	Monster* getMonster() final { return this; }
	const Monster* getMonster() const final { return this; }
	...
	const std::string& getName() const final { return mType->name; }
	const std::string& getNameDescription() const final { return mType->nameDescription; }
	std::string getDescription(int32_t) const final { return strDescription + '.'; }

	CreatureType_t getType() const final { return CREATURETYPE_MONSTER; }

	const Position& getMasterPos() const { return masterPos; }
	void setMasterPos(Position pos) { masterPos = pos; }

	RaceType_t getRace() const final { return mType->info.race; }
	int32_t getArmor() const final { return mType->info.armor; }
	int32_t getDefense() const final { return mType->info.defense; }
	bool isPushable() const final { return mType->info.pushable && baseSpeed != 0; }
	bool isAttackable() const final { return mType->info.isAttackable; }
	...
	bool canSeeInvisibility() const final { return isImmune(CONDITION_INVISIBLE); }
	...
	bool hasExtraSwing() final { return extraMeleeAttack; }
	...
	BlockType_t blockHit(Creature* attacker, CombatType_t combatType, int32_t& damage, bool checkDefense = false, bool checkArmor = false, bool field = false);
...
private:
	...
	uint64_t getLostExperience() const final { return skillLoss ? mType->info.experience : 0; }
	uint16_t getLookCorpse() const final { return mType->info.lookcorpse; }
	void dropLoot(Container* corpse, Creature* lastHitCreature) final;
	uint32_t getDamageImmunities() const final { return mType->info.damageImmunities; }
	uint32_t getConditionImmunities() const final { return mType->info.conditionImmunities; }
	void getPathSearchParams(const Creature* creature, FindPathParams& fpp) const final;
	bool useCacheMap() const final { return true; }
