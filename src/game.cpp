...
void Game::start(ServiceManager* manager)
{
	...
	if (g_config.getBoolean(ConfigManager::DEFAULT_WORLD_LIGHT)) {
		g_gameScheduler.addEvent(createGameTask(EVENT_LIGHTINTERVAL, [this]() { checkLight(); }));
	}
	g_gameScheduler.addEvent(createGameTask(EVENT_CREATURE_THINK_INTERVAL, [this]() { checkCreatures(0); }));
	g_gameScheduler.addEvent(createGameTask(EVENT_DECAYINTERVAL, [this]() { checkDecay(); }));
}
...
void Game::setGameState(GameState_t newState)
{
	...
	switch (newState) {
		...
		case GAME_STATE_SHUTDOWN: {
			...
			g_networkScheduler.addTask([this]() { shutdown(); });

			g_gameScheduler.stop();
			g_databaseScheduler.stop();
			g_networkScheduler.stop();
			break;
		}
                           ...
                           const Position& toPos, uint8_t count)
{
	...
	if (Creature* movingCreature = thing->getCreature()) {
		...
		if (Position::areInRange<1, 1, 0>(movingCreature->getPosition(), player->getPosition())) {
			GameTask* task = createGameTask(MOVE_CREATURE_INTERVAL,
			                                [=, playerID = player->getID(), creatureID = movingCreature->getID()]() {
                              ...
                              Tile* toTile)
{
	if (!player->canDoAction()) {
		...
		GameTask* task =
		    createGameTask(delay, [=, playerID = player->getID(), movingCreatureID = movingCreature->getID(),
	...
	if (!Position::areInRange<1, 1, 0>(movingCreatureOrigPos, player->getPosition())) {
		...
		if (player->getPathTo(movingCreatureOrigPos, listDir, 0, 1, true, true)) {
			...
			GameTask* task = createGameTask(RANGE_MOVE_CREATURE_INTERVAL, [=, playerID = player->getID(),
                          ...
                          const Position& toPos, uint8_t count, Item* item, Cylinder* toCylinder)
{
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		GameTask* task = createGameTask(delay, [=, playerID = player->getID()]() {
	...
	if (!Position::areInRange<1, 1>(playerPos, mapFromPos)) {
		...
		if (player->getPathTo(item->getPosition(), listDir, 0, 1, true, true)) {
			g_networkScheduler.addTask(
			    [=, playerID = player->getID(), listDir = std::move(listDir)]() { playerAutoWalk(playerID, listDir); });
			GameTask* task = createGameTask(RANGE_MOVE_ITEM_INTERVAL, [=, playerID = player->getID()]() {
				playerMoveItemByPlayerID(playerID, fromPos, spriteId, fromStackPos, toPos, count);
			});
			player->setNextWalkActionTask(task);
		} else {
	...
	if (item->isHangable() && toCylinderTile->hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
		...
		if (!Position::areInRange<1, 1, 0>(playerPos, mapToPos)) {
			...
			if (player->getPathTo(walkPos, listDir, 0, 0, true, true)) {
				g_networkScheduler.addTask([=, playerID = player->getID(), listDir = std::move(listDir)]() {
					playerAutoWalk(playerID, listDir);
				});
				GameTask* task = createGameTask(RANGE_MOVE_ITEM_INTERVAL, [this, playerID = player->getID(), itemPos,
                           ...
                           const Position& toPos, uint8_t toStackPos, uint16_t toSpriteId)
{
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (player->getPathTo(walkToPos, listDir, 0, 1, true, true)) {
				...
				GameTask* task = createGameTask(RANGE_USE_ITEM_EX_INTERVAL, [=]() {
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		GameTask* task = createGameTask(delay, [=]() {
...
void Game::playerUseItem(uint32_t playerId, const Position& pos, uint8_t stackPos, uint8_t index, uint16_t spriteId)
{
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
				...
				GameTask* task = createGameTask(RANGE_USE_ITEM_INTERVAL,
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		GameTask* task = createGameTask(delay, [=]() { playerUseItem(playerId, pos, stackPos, index, spriteId); });
		player->setNextActionTask(task);
		return;
	}
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (player->getPathTo(walkToPos, listDir, 0, 1, true, true)) {
				...
				GameTask* task = createGameTask(RANGE_USE_WITH_CREATURE_INTERVAL, [=]() {
	...
	if (!parentContainer) {
		...
		if (it == browseFields.end()) {
			...
			g_gameScheduler.addEvent(
			    createGameTask(30000, [this, position = tile->getPosition()]() { decreaseBrowseFieldRef(position); }));
		} else {
...
void Game::playerRotateItem(uint32_t playerId, const Position& pos, uint8_t stackPos, const uint16_t spriteId)
{
	...
	if (pos.x != 0xFFFF && !Position::areInRange<1, 1, 0>(pos, player->getPosition())) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			g_networkScheduler.addTask(
			    [=, playerID = player->getID(), listDir = std::move(listDir)]() { playerAutoWalk(playerID, listDir); });
			GameTask* task = createGameTask(RANGE_ROTATE_ITEM_INTERVAL,
...
void Game::playerBrowseField(uint32_t playerId, const Position& pos)
{
	...
	if (!Position::areInRange<1, 1>(playerPos, pos)) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			g_networkScheduler.addTask(
			    [=, playerID = player->getID(), listDir = std::move(listDir)]() { playerAutoWalk(playerID, listDir); });
			GameTask* task = createGameTask(RANGE_BROWSE_FIELD_INTERVAL, [=]() { playerBrowseField(playerId, pos); });
			player->setNextWalkActionTask(task);
		} else {
			player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
		}
...
void Game::playerWrapItem(uint32_t playerId, const Position& position, uint8_t stackPos, const uint16_t spriteId)
{
	...
	if (position.x != 0xFFFF && !Position::areInRange<1, 1, 0>(position, player->getPosition())) {
		...
		if (player->getPathTo(position, listDir, 0, 1, true, true)) {
			g_networkScheduler.addTask(
			    [=, playerID = player->getID(), listDir = std::move(listDir)]() { playerAutoWalk(playerID, listDir); });
			GameTask* task = createGameTask(RANGE_WRAP_ITEM_INTERVAL,
                              ...
                              uint16_t spriteId)
{
	...
	if (!Position::areInRange<1, 1>(tradeItemPosition, playerPosition)) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			g_networkScheduler.addTask(
			    [=, playerID = player->getID(), listDir = std::move(listDir)]() { playerAutoWalk(playerID, listDir); });
			GameTask* task = createGameTask(RANGE_REQUEST_TRADE_INTERVAL, [=]() {
...
void Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId)
{
	...
	g_networkScheduler.addTask([this, id = player->getID()]() { updateCreatureWalk(id); });
}
	...
	if (!player->isAccessPlayer()) {
		if (position.x != 0xFFFF && !Position::areInRange<1, 1, 0>(position, player->getPosition())) {
			...
			if (player->getPathTo(position, listDir, 0, 1, true, true)) {
				...
				GameTask* task =
				    createGameTask(400, [=]() { playerRequestEditPodium(playerId, position, stackPos, spriteId); });
				player->setNextWalkActionTask(task);
			} else {
				player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
			}
...
{
	g_gameScheduler.addEvent(
	    createGameTask(EVENT_CHECK_CREATURE_INTERVAL, [=]() { checkCreatures((index + 1) % EVENT_CREATURECOUNT); }));
...
{
	g_gameScheduler.addEvent(createGameTask(EVENT_WORLDTIMEINTERVAL, [this]() { updateWorldTime(); }));
