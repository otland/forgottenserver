                           ...
                           uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count)
{
	...
	if (Creature* movingCreature = thing->getCreature()) {
		...
		if (Position::areInRange<1, 1, 0>(movingCreature->getPosition(), player->getPosition())) {
			SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::MOVE_CREATURE_INTERVAL),
...
void Game::playerMoveCreature(Player* player, Creature* movingCreature, const Position& movingCreatureOrigPos, Tile* toTile)
{
	...
	if (!Position::areInRange<1, 1, 0>(movingCreatureOrigPos, player->getPosition())) {
		...
		if (player->getPathTo(movingCreatureOrigPos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::RANGE_MOVE_CREATURE_INTERVAL), std::bind(&Game::playerMoveCreatureByID, this,
                          ...
                          uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count, Item* item, Cylinder* toCylinder)
{
	...
	if (!Position::areInRange<1, 1>(playerPos, mapFromPos)) {
		...
		if (player->getPathTo(item->getPosition(), listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::RANGE_MOVE_ITEM_INTERVAL), std::bind(&Game::playerMoveItemByPlayerID, this,
			                      player->getID(), fromPos, spriteId, fromStackPos, toPos, count));
			player->setNextWalkActionTask(task);
		} else {
			player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (player->getPathTo(walkToPos, listDir, 0, 1, true, true)) {
				...
				SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::RANGE_USE_ITEM_EX_INTERVAL), std::bind(&Game::playerUseItemEx, this,
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
				...
				SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::RANGE_USE_ITEM_INTERVAL), std::bind(&Game::playerUseItem, this,
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (player->getPathTo(walkToPos, listDir, 0, 1, true, true)) {
				...
				SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::RANGE_USE_WITH_CREATURE_INTERVAL), std::bind(&Game::playerUseWithCreature, this,
...
void Game::playerRotateItem(uint32_t playerId, const Position& pos, uint8_t stackPos, const uint16_t spriteId)
{
	...
	if (pos.x != 0xFFFF && !Position::areInRange<1, 1, 0>(pos, player->getPosition())) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::RANGE_ROTATE_ITEM_INTERVAL), std::bind(&Game::playerRotateItem, this,
...
void Game::playerBrowseField(uint32_t playerId, const Position& pos)
{
	...
	if (!Position::areInRange<1, 1>(playerPos, pos)) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			g_dispatcher.addTask(createTask(std::bind(&Game::playerAutoWalk,
			                                this, player->getID(), std::move(listDir))));
			SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::RANGE_BROWSE_FIELD_INTERVAL), std::bind(
...
void Game::playerWrapItem(uint32_t playerId, const Position& position, uint8_t stackPos, const uint16_t spriteId)
{
	...
	if (position.x != 0xFFFF && !Position::areInRange<1, 1, 0>(position, player->getPosition())) {
		...
		if (player->getPathTo(position, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::RANGE_WRAP_ITEM_INTERVAL), std::bind(&Game::playerWrapItem, this,
                              ...
                              uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	if (!Position::areInRange<1, 1>(tradeItemPosition, playerPosition)) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::RANGE_REQUEST_TRADE_INTERVAL), std::bind(&Game::playerRequestTrade, this,
