...
{
	auto result = tfs::db::store_query(fmt::format("SELECT `name` FROM `guilds` WHERE `id` = {:d}", guildId));
	if (!result) {
		return nullptr;
	}
...
uint32_t IOGuild::getGuildIdByName(const std::string& name)
{
	auto result = tfs::db::store_query(
	    fmt::format("SELECT `id` FROM `guilds` WHERE `name` = {:s}", tfs::db::escape_string(name)));
	if (!result) {
		return 0;
	}
