...
std::shared_ptr<Listener> make_listener(asio::io_context& ioctx, asio::ip::tcp::endpoint endpoint)
{
	asio::ip::tcp::acceptor acceptor{asio::make_strand(ioctx)};
	...
	return std::make_shared<Listener>(ioctx, std::move(acceptor));
}
