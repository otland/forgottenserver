...
WuLine getWuLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1)
{
	WuLine line;
	if (x0 == x1 && y0 == y1) {
		return line;
	...
	if (isSteep) {
		for (int16_t y = x0 + 1; y < x1; y++) {
			int16_t newX = std::floor(interY);
			line.push_back({ newX, y });
			interY += grad;
		}
	}
	else {
		for (int16_t x = x0 + 1; x < x1; x++) {
			int16_t newY = std::floor(interY);
			line.push_back({ x, newY });
			interY += grad;
		}
	}
...
{
	const WuLine line = getWuLine(fromPos.x, fromPos.y, toPos.x, toPos.y);

	for (const auto& it : line) {
		const Tile* tile = getTile(it.first, it.second, fromPos.z);
