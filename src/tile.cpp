...
void Tile::onAddTileItem(Item* item)
{
	...
	const auto& spectators = g_game.map.getSpectators(cylinderMapPos, true);
...
void Tile::removeThing(Thing* thing, uint32_t count)
{
	...
	if (item == ground) {
		...
		const auto& spectators = g_game.map.getSpectators(getPosition(), true);
...
{
	const auto& spectators = g_game.map.getSpectators(getPosition(), true, true);
	for (Creature* spectator : spectators) {
