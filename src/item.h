...
public:
	...
	ItemDecayState_t getDecaying() const { return static_cast<ItemDecayState_t>(getIntAttr(ITEM_ATTRIBUTE_DECAYSTATE)); }
...
public:
	static bool isIntAttrType(itemAttrTypes type) { return (type & 0x7FFE13) != 0; }
	static bool isStrAttrType(itemAttrTypes type) { return (type & 0x1EC) != 0; }
...
public:
	...
	Item* getItem() final { return this; }
	const Item* getItem() const final { return this; }
	...
	void setIntAttr(itemAttrTypes type, int32_t value) { getAttributes()->setIntAttr(type, value); }
	void increaseIntAttr(itemAttrTypes type, int32_t value) { getAttributes()->increaseIntAttr(type, value); }
	...
	bool isPushable() const final { return isMoveable(); }
	int32_t getThrowRange() const final { return (isPickupable() ? 15 : 2); }
	...
	Cylinder* getParent() const { return parent; }
	void setParent(Cylinder* cylinder) { parent = cylinder; }
	Cylinder* getTopParent();
	const Cylinder* getTopParent() const;
	Tile* getTile();
	const Tile* getTile() const;
	bool isRemoved() const { return !parent || parent->isRemoved(); }
