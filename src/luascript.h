...
public:
	...
	// Userdata
	template <class T> static void pushUserdata(lua_State* L, T* value)
	{
	...
	// Get
	template <typename T> static typename std::enable_if<std::is_enum<T>::value, T>::type getNumber(lua_State* L, int32_t arg)
	{
	...
	template <typename T> static T getNumber(lua_State* L, int32_t arg, T defaultValue)
	{
	...
	template <class T> static T* getUserdata(lua_State* L, int32_t arg)
	{
	...
	template <class T> static T** getRawUserdata(lua_State* L, int32_t arg) { return static_cast<T**>(lua_touserdata(L, arg)); }
	...
	template <typename T> static T getField(lua_State* L, int32_t arg, const std::string& key)
	{
