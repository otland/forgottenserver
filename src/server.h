...
{
public:
	bool is_single_socket() const final { return ProtocolType::server_sends_first; }
	bool is_checksummed() const final { return ProtocolType::use_checksum; }
	uint8_t get_protocol_identifier() const final { return ProtocolType::protocol_identifier; }
	const char* get_protocol_name() const final { return ProtocolType::protocol_name(); }

	Protocol_ptr make_protocol(const Connection_ptr& c) const final { return std::make_shared<ProtocolType>(c); }
};
...
template <typename ProtocolType> bool ServiceManager::add(uint16_t port)
{
	...
	if (foundServicePort == acceptors.end()) {
		...
		if (service_port->is_single_socket() || ProtocolType::server_sends_first) {
			std::cout << "ERROR: " << ProtocolType::protocol_name() << " and " << service_port->get_protocol_names() << " cannot use the same port " << port
			          << '.' << std::endl;
			return false;
		}
	}
