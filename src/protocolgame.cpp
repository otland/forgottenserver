...
void ProtocolGame::AddPlayerSkills(NetworkMessage& msg)
{
	...
	msg.add<uint16_t>(player->getLoyaltyMagicLevel()); // base + loyalty bonus(?)
	msg.add<uint16_t>(player->getLoyaltyMagicLevelPercent());

	for (uint8_t i = SKILL_FIRST; i <= SKILL_LAST; ++i) {
		msg.add<uint16_t>(std::min<int32_t>(player->getSkillLevel(i), std::numeric_limits<uint16_t>::max()));
		msg.add<uint16_t>(player->getBaseSkill(i));
		msg.add<uint16_t>(player->getLoyaltySkill(i)); // base + loyalty bonus(?)
		msg.add<uint16_t>(player->getLoyaltySkillPercent(i));
	}
