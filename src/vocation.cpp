...
uint64_t Vocation::getReqSkillTries(uint8_t skill, uint32_t level)
{
	...
	if (level >= skillCache.size()) {
		skillCache.resize(static_cast<size_t>(level) + 9, 0);
	}

	uint64_t& tries = skillCache[level];
	if (tries == 0) {
		tries = std::max<uint64_t>(1, static_cast<uint64_t>(skillBase[skill] * std::pow(skillMultipliers[skill], level - 11)));
	}
	return tries;
}
...
uint64_t Vocation::getReqMana(uint32_t magLevel)
{
	...
	if (magLevel >= cacheMana.size()) {
		cacheMana.resize(static_cast<size_t>(magLevel) + 1, 0);
	}

	uint64_t& tries = manaCache[magLevel];
	if (tries == 0) {
		tries = std::max<uint64_t>(1, static_cast<uint64_t>(1600 * std::pow(manaMultiplier, magLevel - 1)));
	}
	return tries;
}
