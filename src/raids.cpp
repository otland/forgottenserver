...
void Raids::clear()
{
	g_gameScheduler.stopEvent(checkRaidsEvent);
	checkRaidsEvent = 0;
...
void Raid::startRaid()
{
	...
	if (raidEvent) {
		...
		nextEventEvent =
		    g_gameScheduler.addEvent(createGameTask(raidEvent->getDelay(), [=]() { executeRaidEvent(raidEvent); }));
	}
}
...
{
	if (nextEventEvent != 0) {
		g_gameScheduler.stopEvent(nextEventEvent);
		nextEventEvent = 0;
	}
}
