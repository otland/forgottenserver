...
public:
	...
	Player* getPlayer() final { return this; }
	const Player* getPlayer() const final { return this; }
	...
	const std::string& getNameDescription() const final { return name; }
	std::string getDescription(int32_t lookDistance) const final;

	CreatureType_t getType() const final { return CREATURETYPE_PLAYER; }
	...
	bool canSeeInvisibility() const final { return hasFlag(PlayerFlag_CanSenseInvisibility) || group->access; }
	...
	const GuildRank* getGuildRank() const { return guildRank; }
	void setGuildRank(const GuildRank* newGuildRank) { guildRank = newGuildRank; }
	...
	bool hasBlessing(uint8_t value) const { return (blessings & (static_cast<uint8_t>(1) << value)) != 0; }
	...
	int32_t getMaxHealth() const final { return std::max<int32_t>(1, healthMax + varStats[STAT_MAXHITPOINTS]); }
	...
	RaceType_t getRace() const final { return RACE_BLOOD; }
	...
	BlockType_t blockHit(Creature* attacker, CombatType_t combatType, int32_t& damage, bool checkDefense = false, bool checkArmor = false,
	                     bool field = false) final;
	void doAttacking(uint32_t interval) final;
	bool hasExtraSwing() final { return lastAttack > 0 && ((OTSYS_TIME() - lastAttack) >= getAttackSpeed()); }
	...
	// event methods
	void onUpdateTileItem(const Tile* tile, const Position& pos, const Item* oldItem, const ItemType& oldType, const Item* newItem,
	                      const ItemType& newType) final;
	void onRemoveTileItem(const Tile* tile, const Position& pos, const ItemType& iType, const Item* item) final;
...
protected:
	...
	int32_t getStepSpeed() const final { return std::max<int32_t>(PLAYER_MIN_SPEED, std::min<int32_t>(PLAYER_MAX_SPEED, getSpeed())); }
	...
	uint64_t getLostExperience() const final { return skillLoss ? static_cast<uint64_t>(experience * getLostPercent()) : 0; }
	uint32_t getDamageImmunities() const final { return damageImmunities; }
	uint32_t getConditionImmunities() const final { return conditionImmunities; }
	uint32_t getConditionSuppressions() const final { return conditionSuppressions; }
