...
bool Player::editVIP(uint32_t vipGuid, const std::string& description, uint32_t icon, bool notify, const std::vector<uint16_t> groupIds)
{
...
{
	if (VIPGroups.size() >= getMaxVIPGroups()) {
		sendTextMessage(MESSAGE_STATUS_SMALL, "You have already reached the maximum of groups you can create yourself.");
		return false;
	}
