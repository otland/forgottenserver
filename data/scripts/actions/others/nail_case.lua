...
function nailCase.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if math.random(100) >= 80 then
		target:remove()
		player:say(
			"You failed to manicure the nails of the sensitive gravedigger. Before you can make amends, the creature runs away in agony.",
	...
	player:setStorageValue(
		PlayerStorageKeys.nailCaseUseCount,
		player:getStorageValue(PlayerStorageKeys.nailCaseUseCount) + 1
	)
	...
	player:say(
		"You did it! A completely manicured gravedigger is ready to follow you as your own personal trusty mount until the bitter end!",
		TALKTYPE_MONSTER_SAY
	)
	item:remove(1)
	target:remove()
