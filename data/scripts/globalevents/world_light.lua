...
local lightChange = {
	sunrise = math.floor((lightConfig.day - lightConfig.night) / (worldConfig.dayTime - worldConfig.sunrise) * 100)
		/ 100,
	sunset = math.floor((lightConfig.day - lightConfig.night) / (worldConfig.nightTime - worldConfig.sunset) * 100)
		/ 100,
}
...
local function calculateWorldLightLevel()
	...
	if worldTime >= worldConfig.sunrise and worldTime <= worldConfig.dayTime then
		return ((worldConfig.dayTime - worldConfig.sunrise) - (worldConfig.dayTime - worldTime)) * lightChange.sunrise
			+ lightConfig.night
