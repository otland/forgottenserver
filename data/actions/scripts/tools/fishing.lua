...
local p = Player(cid)
	...
	elseif isInArray(waterelementalcorpseIds, itemEx.itemid) == true then -- fish Water elemental Corpse --
		if math.random(1, (100 + (p:getSkillLevel(SKILL_FISHING) / 10))) <= p:getSkillLevel(SKILL_FISHING) then
			...
			if(rand > 995) then
				p:addItem(7632, 1) --  Giant Shimmering Pearl 0,5% --
			elseif(rand > 983) then
				p:addItem(2149, 1) -- small emerald 1,2% --
			elseif(rand > 972) then
				p:addItem(2167, 1) -- energy ring 1,1% --
			elseif(rand > 962) then
				p:addItem(7159, 1) -- Green Perch 1% --
			elseif(rand > 952) then
				p:addItem(2145, 1) -- Small Diamond 1% --
			elseif(rand > 942) then
				p:addItem(7158, 1) -- rainbow trout 1% --
			elseif(rand > 933) then
				p:addItem(2168, 1) -- life ring 0,9% --
			elseif(rand > 924) then
				p:addItem(2143, 1) -- white pearl 0,9% --
			elseif(rand > 916) then
				p:addItem(10220, 1) -- leviathan's amulet 0,8% --
			elseif(rand > 904) then
				p:addItem(2146, 1) -- small sapphire 1,2% --	
			end
			p:addSkillTries(SKILL_FISHING, 1)
			i:transform(10500)
		end
