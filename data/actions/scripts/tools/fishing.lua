...
local fish = {
	...
	[6] = {itemid = 10224, baseChance = 3000} -- Mechanical Fish : 3%
}
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if not target.itemid then return false end
	
	-- Normal Fishing --
	if table.contains(waterWithFish, target.itemid) or table.contains(waterWithoutFish, target.itemid) then
		if targetId ~= 7236 and targetId ~= 7237 then
	...
	-- Water Spirit Fishing --
	if targetId == waterSpiritId then
