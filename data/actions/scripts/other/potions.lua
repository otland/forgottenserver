...
function onUse(player, potion, fromPosition, target, toPosition, isHotkey)
	...
	if stats then
		...
		if stats.health then
			if not doTargetCombatHealth(player.uid, target, COMBAT_HEALING, stats.health.min, stats.health.max, CONST_ME_MAGIC_BLUE) then
				return false
			end
		end

		if stats.mana then
			if not doTargetCombatMana(player.uid, target, stats.mana.min, stats.mana.max, CONST_ME_MAGIC_BLUE) then
				return false
			end
		end
		...
		if potions.removePots then
			potion:remove(1)
			if potions.addEmptyPots and stats.emptyPot then
				-- As in tibia, empty should be put in right place!
				if potion:getTopParent().uid == player.uid then
					player:addItem(stats.emptyPot)
				else
					Game.createItem(stats.emptyPot, 1, potion:getPosition())
				end
			end
		end
	end
