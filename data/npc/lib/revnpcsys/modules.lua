...
function NpcsHandler:travelTo(params)
	for name, dest in pairs(params) do
		...
		toDest:respond(
			string.format("Do you want to travel to {%s} for {%d} gold?", name, dest.money and dest.money[1] or 0)
		)
		...
		for func, parameters in pairs(dest) do
			if func ~= "position" then
				if require[func] then
					...
					print(
						debug.traceback(
							"[Warning - NpcsHandler:travelTo] There is no requirement function with the name: "
								.. func
								.. "\n"
								.. dump(dest),
							1
...
function NpcModules:init(npc, player)
	...
	if self.modItems then
		...
		local backpack = player:getSlotItem(CONST_SLOT_BACKPACK)
				and Container(player:getSlotItem(CONST_SLOT_BACKPACK).uid)
			or nil
