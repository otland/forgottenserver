...
function NpcEvents.onAppear(npc, creature) end
...
function NpcEvents.onSight(npc, creature) end
...
function NpcEvents.onMove(npc, oldPos, newPos) end
...
function NpcEvents.onPlayerCloseChannel(npc, creature) end
...
function NpcEvents.onPlayerEndTrade(npc, creature) end
...
function NpcEvents.onThink(npc)
	...
	for creatureid, releaseTime in pairs(focus.focus) do
		...
		if getDistanceTo(creature:getId()) >= FOCUS.distance or releaseTime < os.mtime() then
			...
			if creature:isPlayer() then
				...
				selfSay(
					handler.farewellResponses[math.random(1, #handler.farewellResponses)]:replaceTags({
	...
	if not next(focus.focus) then
		...
		if creature then
			...
			elseif
				creature:getPosition().y < npc:getPosition().y and creature:getPosition().x == npc:getPosition().x
			then
				selfTurn(DIRECTION_NORTH)
			elseif
				creature:getPosition().x > npc:getPosition().x and creature:getPosition().y == npc:getPosition().y
			then
...
function NpcEvents.onSay(npc, creature, messageType, message)
	...
	if focus:isFocused(creature) then
		...
		for _, word in pairs(handler.farewellWords) do
			if message == word then
				focus:removeFocus(creature)
				closeShopWindow(creature)
				local msg = handler.farewellResponses[math.random(1, #handler.farewellResponses)]:replaceTags({
					playerName = creature:getName(),
				})
				if handler:getTalkState(creature).farewellResponses then
					msg = handler
						:getTalkState(creature).farewellResponses[math.random(
						1,
						#handler:getTalkState(creature).farewellResponses
					)]
						:replaceTags({ playerName = creature:getName() })
				end
	...
	if handler:getTalkState(creature):isKeyword(message) then
		...
		if not focus:isFocused(creature) then
			...
			for _, word in pairs(handler.greetWords) do
				if message == word then
					...
					local msg = handler.greetResponses[math.random(1, #handler.greetResponses)]:replaceTags({
						playerName = creature:getName(),
					})
					if handler:getTalkState(creature).greetResponses then
						msg = handler
							:getTalkState(creature).greetResponses[math.random(
							1,
							#handler:getTalkState(creature).greetResponses
						)]
							:replaceTags({ playerName = creature:getName() })
					end
					talkQueue:addToQueue(creature, msg, TALK.defaultDelay)
		...
		if handler:getTalkState(creature).callback then
			...
			if not ret then
				local msg =
					handler:getTalkState(creature).failureResponse:replaceTags({ playerName = creature:getName() })
				if retMessage then
				...
				if msg == "" then
					print(
						"[Warning - NpcEvents.onSay] Npc: "
							.. npc:getName()
							.. " There is no failureResponse set for keyword: "
							.. message
					)
			...
			if retMessage then
				talkQueue:addToQueue(
					creature,
					retMessage:replaceTags({ playerName = creature:getName() }),
					TALK.defaultDelay
				)
				messageSent = true
			end
			...
			if shop:hasDiscount(creature) then
				...
				for _, item in pairs(items) do
					table.insert(afterDiscount, {
						id = item.id,
						name = item.name,
						buy = (item.buy - math.ceil(
							item.buy / 100 * (shop:hasDiscount(creature) and shop:hasDiscount(creature) or 0)
						)),
						sell = item.sell,
				...
				if type(afterDiscount) == "table" then
					...
					print(
						"[Warning - NpcEvents.onSay] Callback for Npc: "
							.. npc:getName()
							.. " with shop: "
							.. handler:getActiveShop(creature)
		...
		if not greeted then
			...
			if handler:getTalkState(creature):getResponse() and not messageSent then
				local msg =
					handler:getTalkState(creature):getResponse():replaceTags({ playerName = creature:getName() })
				talkQueue:addToQueue(creature, msg, TALK.defaultDelay)
			end
		end
		...
		if focus:isFocused(creature) then
			...
			if not ret then
				if handler:getTalkState(creature):requirements():getFailureRespond(reqType) then
					msg = handler
						:getTalkState(creature)
						:requirements()
						:getFailureRespond(reqType)
						:replaceTags({ playerName = creature:getName() })
				end
				talkQueue:addToQueue(creature, msg, TALK.defaultDelay)
				local _, start = next(handler.keywords)
				...
				if not ret then
					...
					if msg == "" then
						print(
							"[Warning - NpcEvents.onSay] Npc: "
								.. npc:getName()
								.. " has no failureResponse set for keyword: "
								.. message
								.. "."
						)
						print(debug.getinfo(handler:getTalkState(creature).callback).source:match("@?(.*)"))
