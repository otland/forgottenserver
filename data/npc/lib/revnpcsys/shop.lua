			...
			if ItemType(id):getName() ~= "" then
				if ItemType(id):isPickupable() then
					...
					table.insert(self.items, {
						...
						buy = items.buy,
						sell = items.sell,
						subtype = items.subType == nil and nil or items.subType,
					})
				else
					print(
						"[Warning - NpcShop:addItems] item: "
							.. id
							.. " is not pickupable.\n"
		...
		if ItemType(id):getName() ~= "" then
			if ItemType(id):isPickupable() then
				...
				table.insert(self.items, {
					...
					buy = buy,
					sell = sell,
					subtype = subType == nil and nil or subType,
				})
			else
				print(
					"[Warning - NpcShop:addItem] item: "
						.. id
						.. " is not pickupable.\n"
						.. debug.getinfo(2).source:match("@?(.*)")
				)
...
NpcShop.onBuy = function(player, itemid, subType, amount, ignoreCap, inBackpacks)
	...
	if player:getTotalMoney() < totalCost then
		local msg = MESSAGE_LIST.needMoney:replaceTags({
	...
	if a < amount then
		...
		if a > 0 then
			if
				not player:removeTotalMoney((a * (shopItem.buy - math.ceil(shopItem.buy / 100 * discount))) + (b * 20))
			then
				return false
			end
			return true
		...
		local msg = MESSAGE_LIST.bought:replaceTags({
			playerName = player:getName(),
			total = totalCost,
			itemName = shopItem.name,
			amount = amount,
...
NpcShop.onSell = function(player, itemid, subType, amount, ignoreEquipped, _)
	...
	if player:removeItem(itemid, amount, subType, true) then
		local msg = MESSAGE_LIST.sold:replaceTags({
			playerName = player:getName(),
			amount = amount,
			itemName = shopItem.name,
			total = amount * shopItem.sell,
		})
		talkQueue:addToQueue(player, msg, TALK.defaultDelay)
