...
function NpcRequirements:storage(...)
	...
	elseif args == 7 then
		...
		print(
			debug.traceback(
				"[Warning - NpcRequirements:storage] invalid number of arguments: ["
					.. args
					.. "] Function expected 4 or 7 arguments.",
				1
			) .. "\n"
		)
		return false
...
function NpcRequirements:level(...)
	...
	elseif args == 6 then
		...
		print(
			debug.traceback(
				"[Warning - NpcRequirements:level] invalid number of arguments: ["
					.. args
					.. "] Function expected 3 or 6 arguments.",
				1
			) .. "\n"
		)
		return false
...
local function item2(self, item, count, check, failResponse)
	self.requireItem =
		{ items = { item = item, count = count, subType = -1 }, check = check, failResponse = failResponse }
end

local function item3(self, item, count, subType, check, failResponse)
	self.requireItem =
		{ items = { item = item, count = count, subType = subType }, check = check, failResponse = failResponse }
end
...
function NpcRequirements:item(...)
	...
	elseif args == 5 then
		...
		print(
			debug.traceback(
				"[Warning - NpcRequirements:item] invalid number of arguments: ["
					.. args
					.. "] Function expected 3, 4 or 5 arguments.",
				1
			) .. "\n"
		)
		return false
...
local function removeItem1(self, item, count, failResponse)
	self.requireRemoveItem =
		{ items = { item = item, count = count, subType = -1, ignoreEquipped = true }, failResponse = failResponse }
end

local function removeItem2(self, item, count, subType, failResponse)
	self.requireRemoveItem = {
		items = { item = item, count = count, subType = subType, ignoreEquipped = true },
		failResponse = failResponse,
	}
end
...
local function removeItem3(self, item, count, subType, ignoreEquipped, failResponse)
	self.requireRemoveItem = {
		items = { item = item, count = count, subType = subType, ignoreEquipped = ignoreEquipped },
		failResponse = failResponse,
	}
end
...
function NpcRequirements:removeItem(...)
	...
	elseif args == 5 then
		...
		print(
			debug.traceback(
				"[Warning - NpcRequirements:removeItem] invalid number of arguments: ["
					.. args
					.. "] Function expected 3, 4 or 5 arguments.",
				1
			) .. "\n"
...
function NpcRequirements:outfit(outfit1, outfit2, addon, check, failResponse)
	self.requireOutfit =
		{ outfit1 = outfit1, outfit2 = outfit2, addon = addon, check = check, failResponse = failResponse }
end
...
function NpcRequirements:checkStorageValue(player)
	...
	if storage.operator == "==" then
		return player:getStorageValue(storage.key) == storage.value,
			storage.failResponse:replaceTags({ storage = { key = storage.key, value = storage.value } })
	...
	elseif storage.operator == ">" then
		if storage.value2 and storage.operator2 == "<" then
			...
			if player:getStorageValue(storage.key) >= storage.value2 then
				...
				return false,
					storage.failResponse2:replaceTags({ storage = { key = storage.key, value = storage.value2 } })
			end
			return true
	...
	print(
		debug.traceback(
			"[Warning - NpcRequirements:checkStorageValue] Npc: "
				.. Npc(getNpcCid()):getName()
				.. " has operator: "
				.. storage.operator
...
function NpcRequirements:checkLevel(player)
	...
	print(
		debug.traceback(
			"[Warning - NpcRequirements:checkLevel] Npc: "
				.. Npc(getNpcCid()):getName()
				.. " has operator: "
				.. params.operator
				.. " which does not exist.",
...
function NpcRequirements:init(player)
	if self.requireOutfit then
		...
		if
			outfit.check
			and not player:hasOutfit(outfit.outfit1, outfit.addon)
			and not player:hasOutfit(outfit.outfit2, outfit.addon)
		then
			return false, outfit.failResponse
		elseif
			not outfit.check
			and player:hasOutfit(outfit.outfit1, outfit.addon)
			and player:hasOutfit(outfit.outfit2, outfit.addon)
		then
			return false, outfit.failResponse
		end
	end
	...
	if self.requireVocation then
		if
			self.requireVocation.check
			and not table.contains(self.requireVocation.vocation, player:getVocation():getName():lower())
			and not table.contains(self.requireVocation.vocation, player:getVocation():getBase():getName():lower())
		then
			return false,
				self.requireVocation.failResponse:replaceTags({
					vocation = table.concat(self.requireVocation.vocation, ", "),
				})
		elseif
			not self.requireVocation.check
			and table.contains(self.requireVocation.vocation, player:getVocation():getName():lower())
			and table.contains(self.requireVocation.vocation, player:getVocation():getBase():getName():lower())
		then
			return false,
				self.requireVocation.failResponse:replaceTags({
					vocation = table.concat(self.requireVocation.vocation, ", "),
	...
	if self.requireItem then
		for i = 1, #self.requireItem.items do
			...
			if
				self.requireItem.check
				and player:getItemCount(id, index.subType, index.ignoreEquipped) < index.count
			then
				return false, self.requireItem.failResponse:replaceTags({ itemName = name, amount = index.count })
			elseif
				not self.requireItem.check
				and player:getItemCount(id, index.subType, index.ignoreEquipped) >= index.count
			then
				return false, self.requireItem.failResponse:replaceTags({ itemName = name, amount = index.count })
			end
	...
	if self.requireInfight then
		if
			...
			and not player:getCondition(CONDITION_INFIGHT, CONDITIONID_DEFAULT)
			and not player:getCondition(CONDITION_INFIGHT, CONDITIONID_COMBAT)
		then
			return false, self.requireInfight.failResponse
		end
		if
			not self.requireInfight.infight
			and player:getCondition(CONDITION_INFIGHT, CONDITIONID_DEFAULT)
			and player:getCondition(CONDITION_INFIGHT, CONDITIONID_COMBAT)
		then
			return false, self.requireInfight.failResponse
		end
	end
