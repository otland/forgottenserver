...
node1:addChildKeyword(
	...
	{ npcHandler = npcHandler, cost = 20000, level = 20, text = "Congratulations! You are now promoted." }
)
node1:addChildKeyword(
	{ "no" },
	StdModule.say,
	{ npcHandler = npcHandler, onlyFocus = true, text = "Alright then, come back when you are ready.", reset = true }
)
...
local function creatureSayCallback(cid, type, msg)
					...
					if storeInbox then
						...
						if item then
							...
							item:setAttribute(
								...
								"Unwrap it in your own house to create a " .. ItemType(34156):getName() .. "."
							)
	...
	elseif msgcontains(msg, "armor") and npcHandler.topic[cid] == 2 then
		npcHandler:say(
			"So you would like to donate 500.000.000 gold pieces which in return will entitle you to wear a unique armor?",
			cid
		)
		npcHandler.topic[cid] = 3
	elseif msgcontains(msg, "helmet") and npcHandler.topic[cid] == 2 then
		npcHandler:say(
			"So you would like to donate 250.000.000 gold pieces which in return will entitle you to wear a unique helmet?",
			cid
		)
